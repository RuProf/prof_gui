...
def load_profile_data():
    ...
    try:
        current_mtime = os.path.getmtime("/lprof_ext.json")
        if current_mtime > last_mtime:
            with open("/lprof_ext.json", "r") as f:
...
@app.route('/lprof_ext.json')
def serve_profile_json():
    if os.path.exists('/lprof_ext.json'):
        return send_from_directory('/', 'lprof_ext.json')
    return '', 404
